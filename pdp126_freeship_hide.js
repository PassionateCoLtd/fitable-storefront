/* pdp126_freeship_hide.js — 지정 상품(TARGETS) 상세에서만
   새 스킨 내장 무료배송 프로그레스 바(.free_delivery)를 숨김.
   배경(2026-07-15 대표 지시, 126): 새 스킨이 상품상세에 "10만원 이상 구매 시 무료배송 / 40,100원
   ...
   2026-07-31 확장: 165([사전예약] 오브제 와이드 풀업바 톡체크아웃 전용)도 동일 사유로 추가.
   신규 ScriptTag 등록 시 원인불명 422("screen path already has script")로 반영 안 되는 현상이
   있어, 이미 정상 서빙 중인 이 스크립트의 TARGETS를 확장하는 방식으로 처리(신규 슬롯 불필요).
   범위: TARGETS에 명시한 상품 상세에서만(가드). 그 외 상품 영향 0. 스킨 자체는 안 건드림.
   방식: DOM display:none (레이아웃 잔여공간 없이) — 스킨 JS가 늦게/재렌더해도 잡도록 반복 실행.
   롤백: 상품번호를 TARGETS에서 제거 후 재배포, 또는 ScriptTag DELETE 1콜. 전체 try/catch 격리. */
(function () {
  'use strict';
  var TARGETS = ['126', '165'];
  function on126() {
    ...
    return mm && TARGETS.indexOf(mm[1]) !== -1;
  }
  function hide() {
    try {
      if (!on126()) return;
